...
const ProductsScreen = ({route}: any) => {
    ...
    productRef
      ...
      .onSnapshot(snap => {
        if (snap.empty) {
          ...
          snap.forEach(item =>
            items.push({id: item.id, ...item.data()} as ProductModel),
          );
          setProducts(items);
        }
      });
  ...
  return (
    <Container back bigTitle={categoryTitle} isScroll={false}>
      {isLoading ? (
        <Section styles={[globalStyles.center, {flex: 1}, {marginTop: 15}]}>
        ...
        <Section styles={{marginTop: 15}}>
          <FlatList
            ...
            renderItem={({item}) => (
              <ProductItem key={item.id} product={item} navigateTo={'ProductsByOfferListScreen'} />
            )}
          />
        </Section>
      )}
